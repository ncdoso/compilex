...
struct symtab * create_tables_tree_annotation (struct node * ast_root) {
    ...
    while(ast_root) {
        if(!strcmp(ast_root->token, "FieldDecl")) {
            ...
            struct symtab * method_table = build_method_table(ast_root, ast_root->child->sibling);
            ...
            method_line->ast_method_decl = ast_root;
...
char * data_type_to_llvm(enum data_type type){
    switch(type) {
        case DT_UNDEF:
            return "";
        case DT_DECLIT:
